...
const filteredVillages = computed(() => {
    ...
    return (props.villages || []).filter(
        (v) => Number(v.region_id) === Number(filters.region),
    );
});
...
const filteredFarmerGroups = computed(() => {
    ...
    return (props.farmerGroups || []).filter(
        (g) => Number(g.region_id) === Number(filters.region),
    );
});
...
watch(
    ...
    (newRegion, oldRegion) => {
        if (oldRegion !== undefined && newRegion !== oldRegion) {
            const vExists = filteredVillages.value.some(
                (v) => String(v.id) === String(filters.village),
            );
            if (!vExists) filters.village = "";
            const gExists = filteredFarmerGroups.value.some(
                (g) => String(g.id) === String(filters.farmerGroup),
            );
...
<template>
    <div class="relative inline-block text-left">
        <button
            ...
            :class="{
                'border-brand ring-2 ring-brand/20 bg-brand-weak/30':
        ...
        >
            ...
            <div class="mt-4 space-y-4">
                <div>
                    <label class="block text-xs font-bold text-fg mb-1.5"
                        >Kawasan Transmigrasi / Distrik</label
                    >
                ...
                <div>
                    ...
                    >
                    <Select
                        v-model="filters.farmerGroup"
                        class="!min-h-9 text-xs"
                    >
                        <option value="">Semua Kelompok Tani</option>
                        <option
                            v-for="grp in filteredFarmerGroups"
                            :key="grp.id"
                            :value="grp.id"
                        >
                            {{ grp.name }}
            ...
            <div
                class="mt-5 flex items-center justify-between border-t border-border/60 pt-3"
            >
                <button
                    type="button"
