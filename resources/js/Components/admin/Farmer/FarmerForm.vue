...
const form = useForm({
  ...
  region_id: props.initialData?.region?.id ?? (props.initialData?.region_id || ''),
  village_id: props.initialData?.village?.id ?? (props.initialData?.village_id || ''),
  farmer_group_id: props.initialData?.farmer_group?.id ?? (props.initialData?.farmer_group_id || ''),
  commodities: props.initialData?.commodities?.map((c: any) => c.id) || [],
  photo: null as string | null,
})

const isUploadingPhoto = ref(false)
const photoPreviewUrl = ref(props.initialData?.photo?.thumb || props.initialData?.photo?.original || '')
const fileInputRef = ref<HTMLInputElement | null>(null)

const filteredVillages = computed(() => {
  if (!form.region_id) return props.villages || []
  return (props.villages || []).filter((v: any) => Number(v.region_id) === Number(form.region_id))
})

const filteredFarmerGroups = computed(() => {
  if (!form.region_id) return props.farmerGroups || []
  return (props.farmerGroups || []).filter((g: any) => Number(g.region_id) === Number(form.region_id))
})
...
watch(
  ...
  (newRegionId, oldRegionId) => {
    if (oldRegionId !== undefined && newRegionId !== oldRegionId) {
      const villageExists = filteredVillages.value.some((v: any) => Number(v.id) === Number(form.village_id))
      if (!villageExists) form.village_id = ''
      const groupExists = filteredFarmerGroups.value.some((g: any) => Number(g.id) === Number(form.farmer_group_id))
...
const handleSubmit = () => {
  form.transform((data) => ({
    ...
    land_area_ha: data.land_area_ha === '' || data.land_area_ha === null ? null : Number(data.land_area_ha),
    photo: data.photo || null,
  }))

  if (props.isEdit && props.initialData?.id) {
    form.put(`/admin/petani/${props.initialData.id}`, {
      onSuccess: () => {
        toast.success('Profil petani berhasil diperbarui.')
      },
      onError: () => {
        toast.error('Gagal memperbarui data petani. Periksa kembali formulir.')
      },
    })
  } else {
    form.post('/admin/petani', {
      ...
      onError: () => {
        toast.error('Gagal menambahkan petani baru. Periksa kembali formulir.')
      },
    })
  }
}
...
<template>
  <form @submit.prevent="handleSubmit" class="space-y-6">
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-border/60 pb-5">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xs text-fg-muted">
            {{ isEdit ? 'Perbarui identitas, nomor kontak WhatsApp, distrik, dan komoditas budidaya petani.' : 'Lengkapi identitas profil petani baru dan tentukan komoditas budidayanya.' }}
          </p>
        </div>
      </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div class="lg:col-span-2 space-y-6">
        <div class="rounded-2xl border border-border/80 bg-white p-6 shadow-xs space-y-5">
          <div class="flex items-center gap-2 border-b border-border/60 pb-3">
            ...
            <h3 class="text-sm font-bold text-fg">Informasi Petani / Mitra</h3>
          </div>

          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Nama Petani / Mitra" :error="form.errors.name" required>
              <Input
                v-model="form.name"
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Field label="Status Publikasi" :error="form.errors.is_active">
        ...
        <div class="rounded-2xl border border-border/80 bg-white p-6 shadow-xs space-y-5">
          <div class="flex items-center gap-2 border-b border-border/60 pb-3">
            ...
            <h3 class="text-sm font-bold text-fg">Kawasan Transmigrasi &amp; Kelompok Tani</h3>
          </div>
      ...
      <div class="space-y-6">
        <div class="rounded-2xl border border-border/80 bg-white p-6 shadow-xs space-y-5">
          ...
          <div class="space-y-2">
            <label class="block text-xs font-bold text-fg">Pilih Komoditas Panen</label>
            <div class="flex flex-wrap gap-1.5 max-h-52 overflow-y-auto p-2.5 rounded-xl border border-border/80 bg-muted/20">
              <button
                ...
                :class="form.commodities.includes(c.id) ? 'bg-brand text-white shadow-xs' : 'bg-white border border-border text-fg-muted hover:text-fg'"
              >
            ...
            <p class="text-[11px] text-fg-muted">Klik komoditas untuk menandai komoditas yang dibudidayakan oleh petani ini.</p>
          </div>
        </div>
        ...
        <div class="rounded-2xl border border-border/80 bg-white p-6 shadow-xs space-y-5">
          <div class="flex items-center gap-2 border-b border-border/60 pb-3">
            ...
            <h3 class="text-sm font-bold text-fg">Foto Profil Petani</h3>
          </div>
            ...
            <div class="flex flex-col items-center justify-center p-6 rounded-2xl border-2 border-dashed border-border/80 bg-muted/10 text-center gap-3">
              ...
              <div class="space-y-1">
                <div class="text-xs font-semibold text-fg">
                  {{ photoPreviewUrl ? 'Foto profil terunggah' : 'Belum ada foto profil' }}
                </div>
