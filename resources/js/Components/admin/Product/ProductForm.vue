...
const form = useForm({
    ...
    product_category_id:
        props.initialData?.product_category_id ||
        props.initialData?.category?.id ||
        (props.categories?.[0]?.id ?? ""),
    farmer_id:
        props.initialData?.farmer_id ||
        props.initialData?.farmer?.id ||
        (props.farmers?.[0]?.id ?? ""),
    region_id:
        props.initialData?.region_id ||
        props.initialData?.region?.id ||
        props.defaultRegionId ||
        (props.regions?.[0]?.id ?? ""),
    unit_id:
        props.initialData?.unit_id ||
        props.initialData?.unit?.id ||
        (props.units?.[0]?.id ?? ""),
...
type ToggleKey =
    "stock_available" | "is_active" | "is_featured" | "is_region_featured";

const toggleFields: { key: ToggleKey; label: string; description: string }[] = [
    {
        key: "stock_available",
        label: "Stok Tersedia",
        description: "Tandai jika produk ready untuk dibeli",
    },
    {
        key: "is_active",
        label: "Tampilkan di Publik",
        description: "Aktifkan untuk menampilkan di etalase",
    },
    {
        key: "is_featured",
        label: "Unggulan Utama",
        description: "Tampilkan di slider / section favorit",
    },
    {
        key: "is_region_featured",
        label: "Unggulan Kawasan",
        description: "Highlight khusus komoditas khas kawasan",
    },
];
...
const handleSubmit = async () => {
    ...
    try {
        const newFileItems = galleryImages.value.filter(
        ...
        const endpoint =
            props.isEdit && props.initialData?.id
    ...
    <form @submit.prevent="handleSubmit" class="space-y-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 space-y-6">
                <div
                    class="rounded-2xl border border-border/80 bg-white p-5 md:p-6 shadow-xs space-y-5"
                >
                    <div
                        class="flex items-center gap-2.5 border-b border-border/60 pb-3"
                    >
                        <span
                        ...
                        <div>
                            ...
                            <p class="text-xs text-fg-muted">
                                Nama, kategori, dan deskripsi detail produk
                                komoditas
                            </p>
                        </div>
                    </div>
                    ...
                    <Field
                        label="Nama Produk"
                        required
                        helper="Gunakan nama yang jelas dan spesifik (contoh: Beras Mamberamo Merauke 5kg)"
                    >
                        <Input
                            v-model="form.name"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <Field
                            label="Nilai Berat / Takaran"
                            helper="Contoh: 5.00 untuk 5 Kg"
                        >
                            <Input
                    ...
                    <Field
                        ...
                        helper="Jelaskan kualitas panen, rasa, varietas, dan petunjuk penyimpanan"
                    >
                        <Textarea
                ...
                >
                    ...
                    >
                        ...
                        <div>
                            ...
                            <p class="text-xs text-fg-muted">
                                Penentuan harga jual dan satuan takaran
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <Field
                            ...
                            required
                            helper="Otomatis terformat dalam Rupiah"
                        >
                            <div class="relative flex items-center">
                                <span
                                    class="absolute left-3 text-sm font-bold text-fg-muted select-none"
                                    >Rp</span
                                >
                                <Input
                ...
                >
                    ...
                    >
                        ...
                        <div>
                            <h2 class="text-base font-bold text-fg">
                                Asal Usul &amp; Petani Producer
                            </h2>
                            <p class="text-xs text-fg-muted">
                                Kemitraan petani dan lokasi kawasan transmigrasi
                            </p>
                        </div>
                    </div>
            ...
            <div class="space-y-6">
                <div
                    class="rounded-2xl border border-border/80 bg-white p-5 md:p-6 shadow-xs space-y-4"
                >
                    <div
                    ...
                    >
                        <div class="flex min-w-0 items-center gap-2.5">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="text-xs text-fg-muted">
                                    Gambar 1 otomatis menjadi Cover Utama
                                </p>
                            </div>
                        </div>
                        <span
                            class="text-xs font-semibold text-brand bg-brand-weak px-2.5 py-1 rounded-full"
                        >
                    ...
                    <div class="space-y-4">
                        <div
                            ...
                            class="relative overflow-hidden rounded-2xl border-2 border-brand bg-muted/20 aspect-4/3 group shadow-xs"
                        >
                            <img
                            ...
                            <div class="absolute top-3 left-3 z-10">
                                <span
                                    class="inline-flex items-center gap-1.5 rounded-full bg-brand px-3 py-1 text-xs font-extrabold text-white shadow-md"
                                >
                                    <Icon
                                        :icon="Star"
                                        :size="13"
                            ...
                            >
                                <label
                                    class="px-3 py-1.5 rounded-xl bg-white text-xs font-semibold text-fg shadow-md cursor-pointer inline-flex items-center gap-1.5 hover:bg-muted"
                                >
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="grid grid-cols-3 gap-2.5">
                                <div
                                    ...
                                    :class="
                                        index === 0
                                            ...
                                            : 'border-border/80 hover:border-brand/60'
                                    "
                                >
                                    <img
                                        :src="img.url"
                                    ...
                                    <div
                                        class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1.5 p-1"
                                    >
                                        <button
                                            v-if="index !== 0"
                                            type="button"
                                ...
                                <label
                                    class="flex aspect-square flex-col items-center justify-center gap-1 rounded-xl border-2 border-dashed border-border/90 bg-muted/20 p-2 text-center transition-colors hover:border-brand hover:bg-brand-weak/30 cursor-pointer"
                                >
                                    <span
                                        class="flex size-7 items-center justify-center rounded-full bg-brand-weak text-brand"
                                    >
                                        <Icon :icon="Plus" :size="16" />
                                    </span>
                                    <span
                                        class="text-[11px] font-bold text-fg-muted"
                                        >Tambah</span
                                    >
                                    <input
                                        type="file"
                ...
                >
                    ...
                    >
                        ...
                        <div>
                            ...
                            <p class="text-xs text-fg-muted">
                                Pengaturan ketersediaan &amp; tampilan di
                                beranda
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        <template
                            v-for="(toggle, idx) in toggleFields"
                            :key="toggle.key"
                        >
                            ...
                            >
                                <div>
                                    <span
                                        class="text-xs font-bold text-fg block"
                                        >{{ toggle.label }}</span
                                    >
                                    <span
                                        class="text-[11px] text-fg-muted block"
