...
<template>
    <Link :href="`/wilayah/${region.slug}`" class="group block h-full">
        ...
        >
            <div
                class="relative aspect-[2/1] sm:aspect-[16/9] overflow-hidden bg-muted/30"
            >
                <img
                    v-if="region.cover"
            ...
            <div class="flex flex-1 flex-col p-6">
                ...
                <dl class="my-5 grid grid-cols-3 gap-2 sm:gap-3">
                    <div v-for="s in stats" :key="s.label" class="min-w-0">
                        <dt
                            class="truncate text-[11px] sm:text-xs font-medium text-fg-muted"
                        >
                            {{ s.label }}
                        </dt>
                        <dd
                            class="mt-1 text-base sm:text-xl font-bold tabular-nums text-fg"
                        >
                            {{ formatAngka(s.value) }}
                        </dd>
                    </div>
