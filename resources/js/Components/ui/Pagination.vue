...
const perPageList = computed(
    () => props.perPageOptions || [10, 15, 25, 50, 100],
);
...
<template>
    <div
        class="flex flex-col items-center justify-between gap-4 py-3 sm:flex-row border-t border-border/60 pt-4"
    >
        <div
            class="flex flex-wrap items-center gap-3 text-xs font-medium text-fg-muted"
        >
            ...
            <div>
                <template v-if="totalItems > 0 && fromItem && toItem">
                    ...
                    <span class="font-semibold text-fg">{{ toItem }}</span> dari
                    <span class="font-semibold text-fg">{{ totalItems }}</span>
                    data
                </template>
                <template v-else>
                    Total
        ...
        <div class="flex items-center gap-1.5">
            <Component
                ...
                @click="
                    !prevUrl &&
                    currentPage > 1 &&
                    emits('page-change', currentPage - 1)
                "
            >
            ...
            <template v-for="(page, idx) in pageNumbers" :key="idx">
                ...
                <Component
                    :is="
                        getPageUrl(page) && page !== currentPage
                            ? Link
                            : 'button'
                    "
                    v-else
                    ...
                    @click="
                        typeof page === 'number' && emits('page-change', page)
                    "
                >
                    {{ page }}
                </Component>
