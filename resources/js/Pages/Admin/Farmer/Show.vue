...
import { Icon, Badge, Button, AlertDialog, EmptyState } from "@/Components/ui";
...
const executeDelete = () => {
    router.delete(`/admin/petani/${props.farmer.id}`, {
        ...
        onError: () => {
            toast.error(
                "Gagal menghapus petani. Kemungkinan masih ada produk yang terikat.",
            );
        },
    });
};
...
<template>
    ...
    >
        <div class="space-y-6">
            ...
            <div
                class="rounded-2xl border border-border/80 bg-white p-6 shadow-xs"
            >
                <div
                    ...
                    >
                        ...
                        <div class="space-y-2">
                            <div class="flex flex-wrap items-center gap-2">
                                <Badge
                                    :variant="
                                        farmer.is_active ? 'success' : 'neutral'
                                    "
                                    class="text-xs"
                                >
                                    {{
                                ...
                                <span class="text-xs text-fg-muted"
                                    >ID: #PTN-00{{ farmer.id }}</span
                                >
                            </div>

                            <h1
                                class="text-2xl sm:text-3xl font-extrabold text-fg tracking-tight"
                            >
                                {{ farmer.name }}
                            </h1>

                            <div
                                class="flex flex-wrap items-center gap-4 text-xs text-fg-muted pt-1"
                            >
                                <div>
                                ...
                                <div v-if="farmer.created_at">
                                    ...
                                    <span class="font-semibold text-fg">{{
                                        formatTanggal(farmer.created_at)
                                    }}</span>
                                </div>
                            </div>
                        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div
                    class="rounded-2xl border border-border/80 bg-white p-6 shadow-xs space-y-4"
                >
                    <div
                    ...
                    <dl class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
                        <div
                            class="rounded-xl bg-muted/20 p-3 border border-border/60"
                        >
                            <dt class="text-fg-muted font-medium">
                                Distrik / Kawasan
                            </dt>
                            <dd class="font-bold text-fg text-sm mt-1">
                                {{ farmer.region?.name || "-" }}
                            </dd>
                        </div>
                        <div
                        ...
                        >
                            ...
                            <dd class="font-bold text-fg text-sm mt-1">
                                {{ farmer.farmer_group?.name || "Mandiri" }}
                            </dd>
                        </div>
                    </dl>
                </div>
                ...
                >
                    ...
                    <div class="flex flex-wrap gap-2 pt-1">
                        ...
                        >
                            Belum ada komoditas budidaya yang ditandai untuk
                            petani ini.
                        </span>
                    </div>
                </div>
            ...
            >
                ...
                >
                    <div class="flex items-center gap-2.5">
                        ...
                        <div>
                            <h3 class="text-sm font-bold text-fg">
                                Produk Panen Terdaftar
                            </h3>
                            <p class="text-xs text-fg-muted">
                                Daftar komoditas panen yang diproduksi dan
                                dijual oleh petani ini.
                            </p>
                        </div>
                    </div>
                ...
                <div
                    v-if="farmer.products && farmer.products.length > 0"
                    ...
                    <table class="w-full text-left text-sm text-fg">
                        ...
                        <tbody class="divide-y divide-border/60">
                            ...
                            >
                                ...
                                <td
                                    class="px-4 py-3.5 font-semibold text-brand"
                                >
