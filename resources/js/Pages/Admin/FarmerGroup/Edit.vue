...
import { Button, Field, Input, Select, Icon, Badge, EmptyState, AlertDialog } from '@/Components/ui'
...
const handleSubmit = () => {
  form.put(`/admin/kelompok-tani/${props.farmerGroup.id}`, {
    onSuccess: () => {
      toast.success('Kelompok tani berhasil diperbarui.')
    },
    onError: () => {
      toast.error('Gagal memperbarui kelompok tani. Periksa kembali isian Anda.')
    },
  })
}
...
const handleAddMember = () => {
  attachForm.post(`/admin/kelompok-tani/${props.farmerGroup.id}/tambah-petani`, {
    preserveScroll: true,
    onSuccess: () => {
...
const handleRemoveMember = (farmerId: number) => {
  router.post(`/admin/kelompok-tani/${props.farmerGroup.id}/keluarkan-petani`, {
    farmer_id: farmerId
  }, {
    preserveScroll: true,
    onSuccess: () => {
              ...
              <div class="space-y-5">
                <Field label="Distrik / Kawasan" :error="form.errors.region_id" required>
                  ...
                  <p class="text-[11px] text-fg-muted mt-1.5">
                    Mengubah distrik akan memengaruhi daftar calon anggota yang bisa ditambahkan.
                  </p>
                </Field>
        ...
        <div class="lg:col-span-2 space-y-6">
          <div class="rounded-2xl border border-border/80 bg-white shadow-xs overflow-hidden">
            <div class="p-6 border-b border-border/60 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              ...
              <Button size="sm" variant="secondary" class="gap-1.5 font-semibold" @click="openAddMemberModal">
                ...
                <tbody class="divide-y divide-border/60">
                  ...
                  <tr v-for="member in members" :key="member.id" class="hover:bg-muted/20 transition-colors">
                    ...
                    <td class="px-6 py-4 text-right">
                      ...
                      >
                        <template #trigger>
                          <Button size="sm" variant="danger-secondary" class="gap-1 font-semibold">
    ...
    >
      <div class="w-full max-w-md overflow-hidden rounded-card border border-border bg-card shadow-soft animate-in fade-in zoom-in-95 duration-200">
        <div class="flex items-center justify-between border-b border-border px-6 py-4">
          <h3 class="text-base font-bold text-fg">Tambahkan Anggota Petani</h3>
          <button
            type="button"
        ...
        <form @submit.prevent="handleAddMember" class="p-6 space-y-5">
          <div v-if="availableFarmers.length === 0" class="py-4 text-center">
            <EmptyState
          ...
          <div v-else class="space-y-4">
            <Field label="Pilih Petani" :error="attachForm.errors.farmer_id" required>
              <Select v-model="attachForm.farmer_id" required>
                <option value="">Pilih petani yang akan ditambahkan...</option>
                <option v-for="farmer in availableFarmers" :key="farmer.id" :value="farmer.id">
                  {{ farmer.name }} ({{ farmer.phone }})
                </option>
              </Select>
              <p class="text-[11px] text-fg-muted mt-1.5">
                Hanya menampilkan petani yang berada di distrik yang sama dan belum memiliki kelompok tani.
              </p>
            </Field>
          </div>

          <div class="flex justify-end gap-3 pt-2">
            <Button type="button" variant="secondary" @click="closeAddMemberModal">
              Tutup
            </Button>
            <Button v-if="availableFarmers.length > 0" type="submit" :loading="attachForm.processing" class="gap-1.5">
