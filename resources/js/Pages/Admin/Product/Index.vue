...
import { Plus, Search, Edit2, Trash2, Package, ArrowUp, ArrowDown, ArrowUpDown } from '@lucide/vue'
import { Icon, Badge, Button, Pagination, EmptyState } from '@/Components/ui'
...
const productList = computed(() => {
  ...
  return items.map((p: any) => ({
    ...
    price: typeof p.price === 'number' ? `Rp ${new Intl.NumberFormat('id-ID').format(p.price)}` : `Rp ${p.price}`,
    unit: p.unit?.name || 'Kg',
    stock_available: p.stock_available,
    is_featured: p.is_featured,
    is_active: p.is_active,
    image_url: p.photos?.[0]?.thumb || p.photos?.[0]?.original || 'https://images.unsplash.com/photo-1586201375761-83865001e31c?q=80&w=400&auto=format&fit=crop',
  }))
})
...
<template>
  ...
  >
    <template #actions>
      <AdminHeaderAction :icon="Plus" label="Tambah Produk" href="/admin/produk/tambah" />
    </template>
      ...
      <div class="overflow-hidden rounded-2xl border border-border/80 bg-white shadow-xs">
        <div class="overflow-x-auto">
          <table class="w-full text-left text-sm text-fg">
            <thead class="border-b border-border/80 bg-muted/40 text-xs font-bold uppercase tracking-wider text-fg-muted select-none">
              <tr>
                <th
                  scope="col"
            ...
            <tbody class="divide-y divide-border/60">
              ...
              <tr v-for="item in productList" :key="item.id" class="transition-colors hover:bg-muted/20">
                <td class="px-5 py-4">
                  <div class="flex items-center gap-3">
                    <img
                      :src="item.image_url"
                      :alt="item.name"
                      class="size-11 shrink-0 rounded-xl object-cover border border-border/60"
                    />
                    <div class="min-w-0">
                      <p class="font-bold text-fg truncate leading-snug">{{ item.name }}</p>
                      <div class="flex items-center gap-1.5 mt-0.5">
                        <span v-if="item.is_featured" class="rounded bg-accent-weak px-1.5 py-0.2 text-[10px] font-bold text-accent">
                          Unggulan
                        </span>
                        <span class="text-[11px] text-fg-muted">ID: #PRD-00{{ item.id }}</span>
                      </div>
                    </div>
                  </div>
                </td>
                ...
                <td class="px-4 py-4">
                  <p class="text-xs font-semibold text-fg">{{ item.farmer }}</p>
                  <p class="text-[11px] text-fg-muted">{{ item.region }}</p>
                </td>
                <td class="px-4 py-4">
                  <p class="text-xs font-extrabold text-brand">{{ item.price }}</p>
                  <p class="text-[11px] text-fg-muted">{{ item.unit }}</p>
                </td>
                <td class="px-4 py-4">
                  <Badge :variant="item.stock_available ? 'success' : 'danger'">
                ...
                <td class="px-5 py-4 text-right">
                  <div class="flex items-center justify-end gap-1.5">
                    <Link :href="`/admin/produk/${item.id}/edit`">
                      <Button variant="secondary" size="sm" class="size-8 p-0" title="Edit Produk">
