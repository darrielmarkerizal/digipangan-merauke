...
const photoList = computed(() => {
    if (props.product.photos && props.product.photos.length > 0) {
        return props.product.photos
            .map((p) => p.original || p.card || p.thumb)
            .filter(Boolean);
    }
    return [];
});
...
const currentPhoto = computed(() =>
    photoList.value.length > 0
        ? photoList.value[activePhotoIndex.value] || photoList.value[0]
        : null,
);
        ...
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
                <div class="lg:col-span-5 space-y-3">
                    <div
                        class="overflow-hidden rounded-2xl border border-border/80 bg-white p-2 shadow-xs"
                    >
                        <div
                            class="relative aspect-4/3 overflow-hidden rounded-xl bg-muted/30"
                        >
                            <img
                                v-if="currentPhoto"
                                :src="currentPhoto"
                            ...
                            <div
                                v-else
                                class="flex size-full flex-col items-center justify-center gap-2 bg-muted/40 text-fg-muted"
                            >
                                <Icon
                                    :icon="ImageIcon"
                                    :size="48"
                                ...
                                <span
                                    ...
                                    >Tidak ada foto produk</span
                                >
                            </div>
                            <div
                                class="absolute top-3 left-3 flex flex-wrap gap-1.5 z-10"
                            >
                                <Badge
                                    :variant="
                                ...
                                <Badge
                                    :variant="
                                        product.is_active ? 'brand' : 'neutral'
                                    "
                                    class="shadow-sm"
                                >
                                    {{
                ...
                <div class="lg:col-span-7 space-y-6">
                    ...
                    >
                        <div>
                            <div
                                class="flex items-center gap-2 text-xs font-semibold text-fg-muted mb-2"
                            >
                                <span
                                    class="inline-flex items-center gap-1 rounded-md bg-muted px-2 py-0.5 font-bold text-fg"
                                >
                                ...
                                <span class="text-fg-muted"
                                    >ID: #PRD-00{{ product.id }}</span
                                >
                            </div>

                            <h1
                                class="text-2xl font-extrabold text-fg tracking-tight leading-snug"
                            >
                                {{ product.name }}
                            </h1>
                            ...
                            <div class="flex flex-wrap items-center gap-2 mt-3">
                                <span
                                    ...
                                    class="inline-flex items-center gap-1 rounded-lg bg-accent-weak px-2.5 py-1 text-xs font-bold text-accent border border-accent/20"
                                >
                                    <Icon
                                        :icon="Star"
                                        :size="13"
                                ...
                                <span
                                    v-if="product.is_region_featured"
                                    class="inline-flex items-center gap-1 rounded-lg bg-brand-weak px-2.5 py-1 text-xs font-bold text-brand border border-brand/20"
                                >
                        ...
                        <div
                            class="rounded-xl border border-brand/20 bg-brand-weak/30 p-4"
                        >
                            <span
                                class="text-xs font-semibold text-fg-muted uppercase tracking-wider block"
                                >Harga Jual Acuan</span
                            >
                        ...
                        <div class="space-y-2 border-t border-border/60 pt-4">
                            ...
                            >
                                Deskripsi &amp; Mutu Panen
                            </h3>
                            <p
                                class="text-sm text-fg leading-relaxed whitespace-pre-line"
                            >
                                {{
                                    product.description ||
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                <div
                    class="rounded-2xl border border-border/80 bg-white p-5 shadow-xs space-y-3"
                >
                    <div
                    ...
                    >
                        ...
                        <h3 class="text-sm font-bold text-fg">
                            Petani Producer Mitra
                        </h3>
                    </div>
                    <div class="space-y-2 text-xs">
                        <div>
                            <span class="text-fg-muted block"
                                >Nama Petani:</span
                            >
                            <span class="font-bold text-fg text-sm">{{
                                product.farmer?.name || "Petani Mitra"
                            }}</span>
                        </div>
                        <div v-if="product.farmer?.phone">
                            ...
                            >
                            <div
                                class="flex items-center gap-1 font-semibold text-fg"
                            >
                                <Icon
                                    :icon="Phone"
                                    :size="13"
                ...
                >
                    ...
                    <div class="space-y-2 text-xs">
                        <div>
                            <span class="text-fg-muted block"
                                >Kawasan Transmigrasi / Distrik:</span
                            >
                            <span class="font-bold text-fg text-sm">{{
                                product.region?.name || "Kawasan Merauke"
                            }}</span>
                        </div>
                        <div>
                            <span class="text-fg-muted block"
                                >Provinsi / Wilayah:</span
                            >
                            <span class="font-semibold text-fg"
                                >Papua Selatan, Indonesia</span
                            >
                        </div>
                    </div>
                </div>
                ...
                >
                    ...
                    <div class="space-y-2 text-xs">
                        <div>
                            <span class="text-fg-muted block"
                                >Takaran / Nilai Berat:</span
                            >
                            <span class="font-semibold text-fg">
                                {{
                                    product.weight_value
                                        ? `${product.weight_value} ${product.unit?.symbol || product.unit?.name || "Kg"}`
                                        : "-"
                                }}
                            </span>
                        </div>
                        <div v-if="product.created_at">
                            ...
                            >
                            <div
                                class="flex items-center gap-1 font-medium text-fg"
                            >
                                <Icon
                                    :icon="Calendar"
                                    :size="13"
                                ...
                                />
                                <span>{{
                                    formatTanggal(product.created_at)
                                }}</span>
                            </div>
                        </div>
                    </div>
