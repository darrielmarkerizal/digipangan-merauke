...
import HomeNarrativeSection from '@/Components/home/HomeNarrativeSection.vue'
import HomePillarsSection from '@/Components/home/HomePillarsSection.vue'
...
const heroImage = computed(() => heroProduct.value?.photo?.card ?? null)
...
const shownSlugs = computed(
  () =>
    new Set(
      [heroProduct.value?.slug, ...props.featuredProducts.map((p) => p.slug)].filter(
        Boolean,
      ),
    ),
)
...
<template>
  <Head title="DigiPangan Merauke - Marketplace Komunitas Kawasan Transmigrasi" />
